...
import cv2
import numpy as np
# import pytesseract # https://link.zhihu.com/?target=https%3A//github.com/tesseract-ocr/tesseract
...
def parse_pic_to_excel_data(src):
    ...
    for i in range(len(y_point_arr) - 1):
        for j in range(len(x_point_arr) - 1):
            ...
            cv2.waitKey(0)
            '''
            print(x_point_arr[j], y_point_arr[i])
            print(x_point_arr[j+1], y_point_arr[i+1])
            ptLeftTop = (x_point_arr[j], y_point_arr[i])
            ptRightBottom = (x_point_arr[j + 1], y_point_arr[i + 1])
            point_color = (0, 255, 0)  # BGR
            thickness = 1
            lineType = 4
            cv2.rectangle(raw, ptLeftTop, ptRightBottom, point_color, thickness, lineType)
            cv2.imshow("raw", raw)
            cv2.waitKey(0)
